...
const copy = computed(() => isEnglish.value ? {
  ...
  capabilities: [
    { title: 'Web systems', detail: 'Clear platforms built around your operation.', icon: '/icons/expertise-web-systems-3d.png' },
    { title: 'Digital products', detail: 'Useful experiences that make the next step feel natural.', icon: '/icons/expertise-digital-products-3d.png' },
    { title: 'Cloud infrastructure', detail: 'Reliable foundations that are ready to scale.', icon: '/icons/expertise-cloud-infrastructure-3d.png' },
  ],
  stats: [
    { value: '5+', label: 'Years of experience' },
    { value: '20+', label: 'Projects delivered' },
    { value: 'Google Cloud', label: 'Associate Cloud Engineer' },
  ],
  ...
  capabilities: [
    { title: 'Sistemas web', detail: 'Plataformas claras, desenhadas em torno da sua operação.', icon: '/icons/expertise-web-systems-3d.png' },
    { title: 'Produtos digitais', detail: 'Experiências úteis que deixam o próximo passo natural.', icon: '/icons/expertise-digital-products-3d.png' },
    { title: 'Infraestrutura cloud', detail: 'Uma base confiável e pronta para crescer.', icon: '/icons/expertise-cloud-infrastructure-3d.png' },
  ],
  stats: [
...
<template>
  <div class="showcase">
    <section class="showcase__intro" aria-labelledby="intro-title">
      <div class="showcase__container showcase__intro-grid">
        <div>
        ...
        <div class="showcase__highlights">
          <ul class="showcase__capabilities" :aria-label="isEnglish ? 'Areas of expertise' : 'Áreas de atuação'">
            <li v-for="capability in copy.capabilities" :key="capability.title">
              <img :src="capability.icon" alt="" width="72" height="72" loading="lazy" decoding="async" />
              <div>
    ...
    <section class="showcase__section" aria-labelledby="services-title">
      <div class="showcase__container">
        ...
        <div class="showcase__services">
          <article v-for="service in copy.services" :key="service[0]">
...
.showcase__intro, .showcase__section, .showcase__cta { padding: 104px 0; }
.showcase__intro { padding-top: 46px; }
.showcase__intro-grid, .showcase__cta-content { display: grid; grid-template-columns: minmax(0, 1.2fr) minmax(280px, .8fr); gap: clamp(36px, 9vw, 128px); align-items: end; }
...
.showcase__highlights { display: grid; gap: 16px; }
.showcase__capabilities { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin: 0; padding: 0; list-style: none; }
.showcase__capabilities li { display: grid; min-height: 178px; align-content: start; gap: 9px; padding: 16px; border: 1px solid var(--vp-c-divider); border-radius: 18px; background: var(--vp-c-bg); }
.showcase__capabilities img { width: 64px; height: 64px; object-fit: contain; }
.showcase__capabilities strong, .showcase__capabilities span { display: block; }
.showcase__capabilities strong { color: var(--vp-c-text-1); font-size: 12px; font-weight: 750; letter-spacing: -.01em; line-height: 1.3; }
.showcase__capabilities span { margin-top: 5px; color: var(--vp-c-text-2); font-size: 11px; line-height: 1.45; }
...
.showcase__stats dd { margin: 0; color: var(--vp-c-text-1); font-size: clamp(25px, 2.6vw, 34px); font-weight: 750; letter-spacing: -.05em; line-height: 1.05; }
.showcase__stats div:last-child dd { font-size: clamp(19px, 2vw, 25px); letter-spacing: -.035em; }
...
.showcase__case:hover img { transform: scale(1.035); }
...
@media (max-width: 480px) { .showcase__container { width: min(100% - 40px,1152px); }.showcase__capabilities { grid-template-columns: 1fr; }.showcase__capabilities li { min-height: 0; grid-template-columns: 64px 1fr; align-items: center; }.showcase__stats { grid-template-columns: 1fr; }.showcase__stats div { min-height: auto; padding: 20px; }.showcase__stats dd { font-size: 30px; } }
